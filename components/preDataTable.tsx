...
import { preDataSignal } from '@/store/fileStore';
import { CustomPagination } from './custom-pagination';
import { Switch } from './ui/switch';

const allSectors = sectoralIndices;

export default function PreDataTable() {
  useSignals();
  const rawData = preDataSignal.value ?? [];
  ...
  const filteredData = useMemo(() => {
    ...
    if (minPrice !== '') {
      data = data.filter(row => row.open >= parseFloat(minPrice));
    }

    if (maxPrice !== '') {
      data = data.filter(row => row.open <= parseFloat(maxPrice));
    }
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-primary/5">
              ...
              <TableHead className="text-center">Open Price</TableHead>
